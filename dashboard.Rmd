...
n.concesion.poli  = 30
r.ice             = 0.01 ## Tasa Canon Concesión
r.infra           = 0.10
...
```{r concesiones, echo=FALSE, message=FALSE, warning=FALSE}
...
1er escenario {data-navmenu="Resultados por escenario"}
=====================================  

  > Valor base para escenario de demanda `r ifelse(tipo.demanda == "min", "minimo", "maximo")`, en donde el valor total del proyecto es de `r resultado$Sistema$vp.base %>% sum `. 


Row{.tabset}
-----
### Por Sistema {data-height=200}

```{r}

formato.sistema <- function(df){ df %>% knitr::kable(col.names = c("Sistema", "NPV", "TIR"))  }
formato.elemento <- function(df){ df %>% knitr::kable(col.names = c("Elemento", "NPV", "TIR"))  }

resultado$Sistema %>% formato.sistema()
```

### Por Elemento {data-height=200}
...
Row {.tabset}
-----
### Concesion por Sistema {data-height=200}
El valor de las concesiones por lo tanto sería de:
...
### Concesion por Elemento {data-height=200}

```{r}
concesion.elemento %>% filter(Elemento != "Poliductos") %>% 
  mutate(pago = formattable::currency(pago, digits = 0),
                             pago.anticipado = formattable::currency(pago.anticipado, digits = 0)) %>% 
  knitr::kable(col.names = c("Pago", "Pago anticipado","Elemento","Tiempo Concesión"))

```


```{r concesion infraestructura}
...
```{r 2do escenario, warning=FALSE, include=FALSE}
...
2do escenario {data-navmenu="Resultados por escenario"}
========


 > En este escenario existen dos tipos de concesión, del proyecto y de la infraestructura. 
   El valor de la concesion de infraestructura es:

Row
-----
### Valor Concesion Infraestructura
```{r}
canon.infra.sistema %>% 
  mutate(pago = formattable::currency(pago, digits = 0),
                             pago.anticipado = formattable::currency(pago.anticipado, digits = 0)) %>% 
...
Row {.tabset}
-----
El valor del proyecto para SIGSA sería el siguiente

### Por Sistema

```{r}
valor.SIGSA.10$Sistema %>% formato.sistema()
```

### Por Elemento

```{r}
valor.SIGSA.10$Elemento %>% 
  filter(Elemento != "Poliductos") %>% 
  formato.elemento()
```
...
Row {.tabset}
----
El valor del proyecto para el 3ro sería de:

### Por Sistema

```{r}
valor.3ro.10$Sistema %>% formato.sistema()
```
...
3er escenario {data-navmenu="Resultados por escenario"}
========
  > Valorización tercer escenario de negociación
...
```{r}
valor.SIGSA.00$Sistema %>% formato.sistema()
```

### Por Elemento

```{r}
valor.SIGSA.00$Elemento %>% 
  filter(Elemento != "Poliductos") %>% 
  formato.elemento()
```
...
```{r}
valor.3ro.00$Sistema %>% formato.sistema()
```
...
```{r}
valor.3ro.00$Elemento %>% 
  filter(Elemento != "Poliductos") %>% 
  formato.elemento()
```


Resumen {data-navmenu="Otros"}
=====
...
Info. Financiera {data-navmenu="Otros"}
======================
...
### WACC
```{r}
valueBox(wacc %>% percent( ), color = "#EFF5FB")
```

### Retorno esperado accionistas

```{r}
valueBox(expected.return %>% percent(), color = "#EFF5FB")
```

### Tasa financiamiento

```{r}
valueBox(r %>% percent, color = "#EFF5FB")
```

Row{.tabset}
----
### Información calculo retorno esperado

```{r}
    
tmp <-  financial.data %>% 
  mutate(rf = percent(rf),
         rm = percent(rm),
         riesgo.pais = "3.55%",
         default = "2.89%") %>% 
